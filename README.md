...
เวอร์ชัน 7 | รูปแบบ Jupyter Notebook
...
ออกแบบมาสำหรับการวัดผล 2–5 ระยะเวลา (Periods) และในเวอร์ชันนี้มีการเพิ่ม **Session 7a** สำหรับทำนาย (Forecast) ค่า RMS / CF / Kurtosis ของ period ถัดไปด้วย Linear Regression
...
4. **Trend Analysis & Forecast** — วิเคราะห์ slope และคาดการณ์ period ถัดไป
...
สรุปสภาพเครื่องพร้อม flag เตือน และ linear slope (ทิศทางแนวโน้ม):
1. ISO Zone + ค่า CF/RMS/Peak/Kurtosis พร้อม flag
2. Linear slope ของ CF / RMS / Kurtosis
3. Top 3 dominant frequencies ต่อ period
4. คำแนะนำการซ่อมบำรุง

### Session 7a — RMS / CF Forecast *(ใหม่)*
**ทำนายค่าของ period ถัดไป** ด้วย `sklearn.LinearRegression`:

| Metric | ข้อมูลที่แสดง |
|---|---|
| RMS (G) | Historical + Predicted + Δ% vs last |
| Peak (G) | Historical + Predicted + Δ% vs last |
| Crest Factor | Historical + Predicted + Risk flag |
| Kurtosis | Historical + Predicted + Risk flag |

ผลลัพธ์เก็บใน `forecast_results` (dict) เพื่อส่งต่อ Session 8

### Session 8 — Build DataFrames, Forecast Summary & Export CSV
จัดทำ DataFrame 6 ชุด แล้วส่งออกไฟล์ `output/vibration_full_report.csv`:
...
- Trend/Forecast (Linear Regression) ควรมีข้อมูลอย่างน้อย 10 periods เพื่อความน่าเชื่อถือ — 2–3 periods แสดงทิศทางเบื้องต้นเท่านั้น
